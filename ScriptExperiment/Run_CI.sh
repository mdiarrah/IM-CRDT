...
ssh root@$MASTER "sh -c 'cd CRDT_IPFS && ./IPFS_CRDT --mode BootStrap --name node1 --updatesNB $numberUpdates --updating true > /dev/null & '"  &
sleep 30s
...
then
echo "updating"
ssh root@$SLAVE "sh -c 'cd CRDT_IPFS && ./IPFS_CRDT --mode update --ni ${BOOTSTRAPID} --name node1 --updatesNB $numberUpdates --updating true > /dev/null &'" &
x=$(( $x - 1 ))
else
echo "NOT updating"
ssh root@$SLAVE "sh -c 'cd CRDT_IPFS && ./IPFS_CRDT --mode update --ni ${BOOTSTRAPID} --name node1 --updatesNB $numberUpdates > /dev/null &'" &
fi
