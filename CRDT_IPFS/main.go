...
func main() {
	...
	if *mode == "BootStrap" {
		...
		Tests.Peer1Concu(*peerName, *updatesNB, *ntpServ) // ------------- MANAGE CONCURENCY !!!
		...
		if *updating {
			...
			Tests.Peer2ConcuUpdate(*peerName, *bootstrapPeer, *updatesNB, *ntpServ) // ------------- MANAGE CONCURENCY !!!
		} else {
			fmt.Println("NOT UPDATING FIOU")
			Tests.Peer2Concu(*peerName, *bootstrapPeer, *updatesNB, *ntpServ) // ------------- MANAGE CONCURENCY !!!
		}
